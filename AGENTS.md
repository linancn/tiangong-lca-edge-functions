...
- `pnpm check` validates exact runtime versions, checks all 152 enabled function/test roots through one bounded shared Deno graph, runs 73 Node contract tests, and executes 512 default Deno behavior tests plus one opt-in live Upstash test that remains ignored without explicit credentials
