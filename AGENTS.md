...
lastReviewedNote: 'Reviewed for Edge #363: legacy LCA user API-key/Cognito bearer acceptance and generic auth Redis are removed; claims-first Supabase/service auth and isolated Portal/embedding boundaries remain.'
related:
  - .docpact/config.yaml
...
- `pnpm check` validates exact runtime versions, checks all 155 enabled function/test roots through one bounded shared Deno graph, runs 73 Node contract tests, and executes 499 default Deno behavior tests plus one opt-in live Upstash test that remains ignored without explicit credentials
