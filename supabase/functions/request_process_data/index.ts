...
Deno.serve(async (req: Request): Promise<Response> => {
    ...
    try {
        ...
        if (!exchanges || !Array.isArray(exchanges)) {
            return new Response(
                JSON.stringify({ error: "No exchanges found in process data" }),
                { 
                    headers: { ...corsHeaders, "Content-Type": "application/json" },
                    status: 404 
                }
        ...
        return new Response(
            JSON.stringify({ error: "Request Process Data Internal Server Error", details: error instanceof Error ? error.message : String(error) }),
            { 
                headers: { ...corsHeaders, "Content-Type": "application/json" },
                status: 500 
            }
