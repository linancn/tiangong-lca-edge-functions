...
export const ANT_CHAIN_KEY = JSON.parse(
    atob(Deno.env.get("ANT_CHAIN_KEY") ?? ""),
);
...
const supabase = createClient(
    ...
    Deno.env.get("REMOTE_SUPABASE_ANON_KEY") ??
...
export const dynamicParam = {
    ...
    "operatorList": [{
        ...
        "dynamicParameter": {
            ...
            "result_table_create_cmd":
                "CREATE table result_table (average_value double)",
            "query_cmd":
...
export function updateCustomParams(
    ...
    if (
        operator.dynamicParameter &&
        "custom_params" in operator.dynamicParameter
    ) {
        ...
        console.warn(
            `Operator with ID ${operatorId} does not have custom_params`,
        );
    }
...
export function batchUpdateCustomParams(
    ...
    for (const update of updates) {
        updatedParams = updateCustomParams(
            updatedParams,
            update.operatorId,
            update.customParams,
        );
...
export function createDynamicParamWithDatasetValues(
    ...
    return batchUpdateCustomParams(
        dynamicParam,
        [
            {
...
export async function getCoDatasetId(params: Record<string, any>) {
    ...
    const resultInfoList: Record<string, any>[] =
        instanceInfo.data.resultInfoList;
    const coDatasetId = resultInfoList.find((item: Record<string, any>) =>
        item.status === "SUCCESS"
    )?.coDatasetId;
    return coDatasetId;
}
...
export async function runCalculationWorkflow(
    ...
    while (retryCount < maxRetries) {
        try {
            console.log(
                `Attempt ${
                    retryCount + 1
                } of ${maxRetries} to run calculation workflow`,
            );

            // Step 1: Create dynamic parameters with the provided dataset values
            const dynamicParam = createDynamicParamWithDatasetValues(
                dataSetParams1,
                dataSetParams2,
            );
            ...
            if (!createResponse.success) {
                console.error(
                    "Failed to create calculation instance:",
                    createResponse,
                );
                retryCount++;
                continue;
            ...
            while (true) {
                ...
                if (appInstanceStatus === "FAILED") {
                    console.error(
                        `Calculation failed with status: ${
                    ...
                    console.log(
                        `Current status: ${JSON.stringify(status.status)}`,
                    );
                }
            ...
            console.error(
                `Error during calculation workflow (attempt ${
                    retryCount + 1
                }):`,
                error,
            );
    ...
    console.error(
        `All ${maxRetries} attempts to run calculation workflow failed`,
    );
    return null;
}
