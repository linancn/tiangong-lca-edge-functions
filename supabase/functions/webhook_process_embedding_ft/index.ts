...
import { projectProcessSearchText } from '../_shared/search_text_projection.ts';
...
export const tidasProcessToMarkdown = (processJson: any, lang = DEFAULT_LANG) => {
  ...
  Deno.serve(async (req) => {
    ...
    try {
      ...
      for (const [index, payload] of events.entries()) {
        ...
        const searchText = projectProcessSearchText(jsonData, id);
