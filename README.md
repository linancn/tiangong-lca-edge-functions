...
The Issue #249 cutover contract is validated against `database-engine` commit `6809528c32bac8163e9a6eec9b985d57370589e1`, whose migration head is `20260801060304` and whose Worker physical/API expansion is introduced by `20260801060304_issue_356_worker_control_plane_physical_expand.sql`. The hosted test requires `WORKER_CAPABILITY_SUPABASE_ACCESS_TOKEN` as a read-only attestation credential; caller-provided expected commit or migration-head values are not accepted. Before creating test users or jobs, it resolves the exact Supabase parent/branch UUID/Preview ref, migration API receipt and read-only ledger, migration-generated private residue view definition and ACL, then binds the same source branch to merged database-engine PR `#365`, its exact commit, and the migration-file SHA through GitHub. The capability repository always selects the exposed `api` schema explicitly; default-schema `public.worker_*` compatibility wrappers are not Edge runtime contracts. Deploy only after an equivalent or newer database head containing the versioned API adapters is present. After a database reset or migration, confirm the six `api.worker_*_v1` routines appear in the service-role PostgREST schema profile; reload the isolated PostgREST schema cache if it retained a pre-migration catalog.
