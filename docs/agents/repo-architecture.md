...
`scripts/schema-boundary-consumer-audit.ts` has two profiles: `expand` rejects unregistered dependencies while allowing explicitly non-authorizing migration residue, and `contract` fails until every non-core public dependency has moved behind an approved boundary. The Edge manifest has a committed JSON Schema with a bound SHA-256 so database-engine can validate both artifacts from the exact Edge commit. A database-engine #357 v3 candidate is recorded with exact provenance, structured exposure signatures, and unresolved ACL fields, but it cannot authorize migration. Contract success additionally requires a reviewed-frozen v3 manifest/sidecar, content and exposure fingerprints, exact commit/review receipt, and complete authorization-policy bindings.
