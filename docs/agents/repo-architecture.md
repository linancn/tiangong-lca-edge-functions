...
lastReviewedNote: 'Reviewed for Issue #233: Data Product downloads require the bounded XLSX or machine-manifest selector, use the two-argument actor RPC, cap signing at artifact expiry, and preserve locator-free availability plus opaque denial boundaries.'
related:
  - ../../AGENTS.md
  - ../../.docpact/config.yaml
  - ./repo-validation.md
...
`app_data_product_commands` is the JWT-only command boundary for Data Product scope-closure checks and result-build requests. It forwards only user scope intent to actor-bound database RPCs; the database derives snapshot, policy, certificate, and artifact-lifecycle bindings. The shared data-product repository preserves the database-owned versioned check/issues/feed projections while explicitly decoding the check's fixed-order, locator-free artifact availability summaries. For downloads, the strict public request requires exactly `closure_report_xlsx` or `closure_issue_manifest`, forwards that selector to the database's two-argument actor RPC, and signs only a matching ready, unexpired descriptor. Partition selectors are not part of this public endpoint. Signed URLs are capped at both 900 seconds and the remaining artifact lifetime and use the database-provided semantic filename. Owner-visible expiry maps to a stable `410`, while unavailable, unauthorized, deleted, unready, and integrity-invalid artifacts remain one opaque `404`. The service client may see the private bucket/path solely for the signing step and never returns either field or storage error details to the browser. Task feed visibility is database-owned ACL, not a consequence of task-center category or presenter metadata.
