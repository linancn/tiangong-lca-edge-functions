...
lastReviewedNote: 'Updated for Issue #250: schema-boundary authorization uses exact adapter/residue syntax tuples and remains limited to the current save-draft slice.'
related:
  - ../../AGENTS.md
  - ../../.docpact/config.yaml
  - ./repo-validation.md
...
`supabase/functions/_shared/capabilities/schema_boundary.ts` is the shared PostgREST entrypoint for database-owned `api` capabilities. Actor and service routines use separate literal-union capability maps; relation callers also select a capability ID instead of passing a physical relation string. The adjacent versioned manifest binds the reviewed database inventory, the nine public core tables, exact current residue, dynamic SQL/PGMQ/Storage consumers, and the field/filter/order/limit/ownership/transaction requirements that database-engine must satisfy. The audit parses TypeScript syntax to derive the 48 in-scope relation occurrences and actor/service capability calls independently, then requires bidirectional equality with the manifest. RPC/schema authorization is syntax-first: five typed-adapter call tuples, thirteen direct legacy public-residue tuples, seven detached method-guard residue tuples, and one non-database computed-call exception are exact-counted; additions, deletion, or shape drift fail. File-local alias resolution remains supporting evidence for relation inventory only, not a global database-consumer proof.
