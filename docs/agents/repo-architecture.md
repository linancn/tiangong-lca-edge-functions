...
lastReviewedNote: 'Reviewed while integrating Edge #363 and #369: shared auth is Supabase JWT/service-only and Redis is Portal-only, while concurrent provider ordering, deadline, telemetry, and cleanup semantics remain intact.'
related:
  - ../../AGENTS.md
  - ../../.docpact/config.yaml
  - ./repo-validation.md
...
| `supabase/functions/_shared/portal_hybrid_deadline.ts` | stable | one absolute 6-second Edge handler-entry deadline with two seconds of BFF headroom, shared model/database AbortSignal, remaining-time operation caps, and non-blocking bounded cleanup |
