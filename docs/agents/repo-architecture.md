...
lastReviewedNote: 'Updated for Issue #250: non-direct RPC/schema syntax is default-denied and ordinary schema fields use exact-counted tuples; the rule remains limited to the current slice.'
related:
  - ../../AGENTS.md
  - ../../.docpact/config.yaml
  - ./repo-validation.md
...
`supabase/functions/_shared/capabilities/schema_boundary.ts` is the shared PostgREST entrypoint for database-owned `api` capabilities. Actor and service routines use separate literal-union capability maps; relation callers also select a capability ID instead of passing a physical relation string. The adjacent versioned manifest binds the reviewed database inventory, the nine public core tables, exact current residue, dynamic SQL/PGMQ/Storage consumers, and the field/filter/order/limit/ownership/transaction requirements that database-engine must satisfy. The audit parses TypeScript syntax to derive the 48 in-scope relation occurrences and actor/service capability calls independently, then requires bidirectional equality with the manifest. RPC/schema authorization is syntax-first: five typed-adapter direct-call tuples, thirteen direct legacy public-residue tuples, and seven independent `typeof` method-guard residue tuples are exact-counted. Every other non-direct `.rpc`/`.schema` property or element access is rejected, including aliases and `bind`/`call`/`apply` chains. Thirteen ordinary business-field `schema` references are confined to twelve exact path/symbol/expression/context tuples (one tuple has count two), and one non-database computed-call exception is separately exact-counted; additions, deletion, duplication, or shape drift fail. File-local alias resolution remains supporting evidence for relation inventory only, not a global database-consumer proof.

`scripts/schema-boundary-consumer-audit.ts` has two profiles: `expand` rejects unregistered dependencies while allowing explicitly non-authorizing migration residue, and `contract` fails until every non-core public dependency has moved behind an approved boundary. The Edge manifest has a committed JSON Schema with a bound SHA-256 so database-engine can validate both artifacts from the exact Edge commit. Incremental vertical slices keep separate API-ready and public-preservation capability maps: only a routine present at the bound database merge/migration head may select `api`, while every deferred actor/service routine stays on the unqualified public transport and remains visible in contract findings. The scanner authorizes only the currently bound save-draft slice and does not claim global consumer-zero. A database-engine #357 v3 candidate is recorded with exact provenance, structured exposure signatures, and unresolved ACL fields, but it cannot authorize the full migration. Contract success additionally requires a reviewed-frozen v3 manifest/sidecar, content and exposure fingerprints, exact commit/review receipt, and complete authorization-policy bindings.
...
This repository has a versioned local `pre-push` hook under `.githooks/pre-push` that delegates to `scripts/docpact-gate.sh` and then runs non-mutating `npm run lint` plus `npm run check`. The hook aborts if the lint step changes the working tree, so generated formatting changes must be reviewed and committed before push. The gate resolves the CLI through `scripts/docpact`, so local agent shells do not need bare `docpact` on `PATH`. The hook is the local guard for docpact config validation, enforced doc-governance linting, and Edge Function checks. The GitHub `CI` workflow runs on pull requests to `dev` or `main`, checks out the exact pull-request head SHA, verifies its tree identity, and also supports manual dispatch.
