...
lastReviewedNote: 'Reviewed after Issue #313 isolated the Portal project-bound public key, explicit model/AWS provider injection, and per-route deployment provenance from existing Functions.'
related:
  - ../../AGENTS.md
  - ../../.docpact/config.yaml
  - ./repo-validation.md
...
| `supabase/functions/_shared/portal_public_transport.ts` | stable | dedicated current-project publishable-key resolution, exact inbound/legacy-anon transport, Cookie rejection, safe Supabase URL, and bounded raw-byte reader shared by signed Portal routes |
