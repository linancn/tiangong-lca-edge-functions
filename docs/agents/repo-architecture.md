...
lastReviewedNote: 'Reviewed for Issue #233: Data Product downloads strictly decode ready, unexpired XLSX and machine manifest/partition descriptors, cap signing at artifact expiry, and preserve opaque denial plus private locator boundaries.'
related:
  - ../../AGENTS.md
  - ../../.docpact/config.yaml
  - ./repo-validation.md
...
`app_data_product_commands` is the JWT-only command boundary for Data Product scope-closure checks and result-build requests. It forwards only user scope intent to actor-bound database RPCs; the database derives snapshot, policy, certificate, and artifact-lifecycle bindings. The shared data-product repository returns the database-owned versioned check/issues/feed projections unchanged. For downloads, it strictly decodes the actor-bound artifact descriptor, accepts only the ready human XLSX report or complete-machine manifest/partition roles, caps the signed URL at both 900 seconds and the remaining artifact lifetime, and sets the database-provided semantic filename. Owner-visible expiry maps to a stable `410`, while unavailable, unauthorized, deleted, and unready artifacts remain one opaque `404`. The service client may see the private bucket/path solely for the signing step and never returns either field or storage error details to the browser. Task feed visibility is database-owned ACL, not a consequence of task-center category or presenter metadata.
