...
| PostgREST schema boundary, capability manifest, or public-residue cutover | `npm run audit:schema-boundary:expand`; `deno test --allow-read --config supabase/functions/deno.json test/schema_boundary_capability_test.ts test/schema_boundary_consumer_audit_test.ts`; `npm run check`; targeted tests for every migrated consumer | run `npm run audit:schema-boundary:contract` as the fail-closed completion gate; with the exact coordinated database-engine migration in an isolated loopback stack, prove actor/service/anon ACL, ownership, idempotency, CAS, and transaction semantics | `expand` may pass only when every remaining dependency is explicitly inventoried; it must not claim consumer-zero. `contract` must remain red until no non-core public relation/routine residue remains. Candidate DB routine names are not an Edge contract until database-engine freezes them. |
