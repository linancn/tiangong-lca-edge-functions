...
lastReviewedNote: 'Reviewed for Issue #258: result-family proof now includes strict DTO decoding, consumer-zero, command concurrency, handler parity, and a fail-closed loopback Data API contract.'
related:
  - ../../AGENTS.md
  - ../../.docpact/config.yaml
...
| Command runtime, command handlers, capability adapters, or DB-RPC wrappers | `npm run lint`; `npm run check`; targeted `deno check` on changed `_shared/command_runtime/**`, `_shared/commands/**`, `_shared/capabilities/**`, `_shared/db_rpc/**`, and at least one direct consumer | run nearby repo tests such as `test/command_runtime_test.ts`, `test/dataset_command_rpc_contract_test.ts`, or `test/review_command_rpc_contract_test.ts` | Schema-bound adapters must prove a distinct scoped fake, request-JWT/service-role type rejection, zero root RPC calls, no caller-selected schema/routine, and no default-schema fallback. If the change depends on new SQL or RPC truth, record the `database-engine` follow-up explicitly. |
...
| LCA solve, queue, result, or scope helpers | `npm run lint`; `npm run check`; targeted `deno check` on changed `lca_*` files and `_shared/lca_*` helpers; run neighboring scope/queue tests. Result-family changes must run `npm run test:lca-result-family:unit` and `npm run test:lca-result-family:consumer-zero`; these prove strict projection/cache/command decoders, sanitized HTTP parity, exact touch/admit/reconcile sequences, result-pending and two-poll terminal convergence, canonical reused bindings, all-unit `replaceReady=true` race behavior, and the exact 15 relation / 3 old RPC / 0 factorization legacy baseline reaching zero. Snapshot capability changes must run `test/lca_snapshot_capability_test.ts`, `test/lca_snapshot_endpoint_handler_test.ts`, `npm run diagnose:lca-snapshot:call-sites`, `test/lca_snapshot_scope_db_test.ts`, and `test/lca_snapshot_build_queue_test.ts`. The handler tests invoke production factories and prove exact repository operations, DTO consumption, candidate fallback, and 404/500 behavior. Authorization evidence comes from real handler behavior, Data API role tests, ACL/readback proof, runtime observation, and owner confirmation. When the exact database-engine stack is available, configure the loopback-only `LCA_SNAPSHOT_CONTRACT_*` or `LCA_RESULT_CONTRACT_*` environment and run `npm run test:lca-snapshot:db` or `npm run test:lca-result-family:db`. The result runner requires the exact DB merge commit and migration head, rejects a receipt that differs from the adapter pin, reads the actual migration ledger head, exercises all eight service-role calls and the real jobs/results/contribution-result/query handlers, proves exact `42501` for anon and a real authenticated session across all eight calls, cleans fixtures/users/sessions, independently reads residue as zero, and rejects ignored or partially executed tests. Static LCIA contract changes must also run `test/lca_static_cache_bundle_contract_test.ts`, `test/lca_snapshot_scope_test.ts`, and `test/lca_all_unit_solve_queue_test.ts` with `--allow-read`. For worker_jobs cutover changes also run `test/worker_jobs_cutover_test.ts` and `test/worker_jobs_test.ts` | run `scripts/lca_submit_poll_fetch.sh` when the task explicitly touches the submit, poll, or fetch path; otherwise record why that proof is deferred | Snapshot and result-family access are service-role-only through fixed `api` adapters, with no request-JWT substitution, physical relation, ninth query, private/public schema, or compatibility fallback. After a result-cache read, one request may touch, admit, or reconcile only. `result_pending` never enqueues. Contribution failed/stale/cancelled recovery intentionally reconciles on the first poll and admits on the next; database-engine Issue #395 must normalize cancelled to failed. `worker_jobs` is the default enqueue path; `LCA_WORKER_JOBS_ENABLED=false` must fail closed. Domain rows/cache remain result metadata, not task fact. Missing DB-side truth is validated in `database-engine`, not here. |
