...
lastReviewedNote: 'Reviewed for Issue #304: the canonical gate now reports Deno 2.9.5 with bundled TypeScript 6.0.3, checks one bounded 139-root graph, and runs all 367 behavior tests under minimum permissions.'
related:
  - ../../AGENTS.md
  - ../../.docpact/config.yaml
...
`pnpm check` first requires exact Deno `2.9.5` / bundled TypeScript `6.0.3` plus Node `24.19.0` / pnpm `11.23.0`. It then checks all 139 enabled `supabase/functions/*/index.ts` and `test/*.ts` roots in one shared graph, runs Node contract tests, and executes all Deno behavior tests with only env, read, and loopback-network permissions. Deno is the authoritative compiler; no npm TypeScript package participates.
