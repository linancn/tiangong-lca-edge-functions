...
lastReviewedNote: 'Reviewed after Issue #316 added the isolated R0 verifier, optional-secret normalization, loopback Redis fixtures, and live Main-parent Preview branch verification for deploy and cleanup.'
related:
  - ../../AGENTS.md
  - ../../.docpact/config.yaml
...
| Disposable Portal R0 HMAC/Redis interoperability fixture | `pnpm lint`; `pnpm check`; `deno test --allow-env --allow-read --allow-net=127.0.0.1 --config supabase/functions/deno.json test/portal_r0_hmac_transport_test.ts test/portal_r0_redis_test.ts test/portal_r0_redis_live_fixture_test.ts test/portal_r0_hmac_verify_v1_test.ts test/portal_r0_deploy_contract_test.ts`; `node --test scripts/deploy-portal-r0-fixture.test.cjs scripts/cleanup-portal-r0-fixture.test.cjs`; targeted `deno check` on `portal_r0_hmac_verify_v1` and `_shared/portal_r0_*` | from one clean exact SHA, use only the exact nondefault/nonpersistent/no-data Supabase Preview branch returned live by `branches list --output-format json` from the configured Main parent and bound to explicit branch name/Git branch/optional PR number, one-time credentials, dedicated Redis, and `portal:r0:<fixture>:v1`; capture interoperability and rotation, run guarded cleanup, then externally delete credentials and Redis | Remote deploy is Preview-only and requires `FUNCTIONS_DEPLOYED` plus `ACTIVE_HEALTHY`; 403, malformed, and empty responses fail closed. `test` remains local-only. Current/previous require distinct IDs and constant-time-distinct secrets; both-empty optional values mean absent while one-sided empty fails. HMAC precedes transport, nonce, Lua, and JSON. Persistent deploy scripts reject this slug. Deploy and cleanup share live branch and clean-SHA checks but use separate acknowledgements. Deploy requires future expiry within 24 hours; cleanup permits expired resources. An absent target in a valid nonempty Main-parent response is a verified terminal and emits only the external Redis/credential checklist. |
