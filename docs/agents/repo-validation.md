...
lastReviewedNote: 'Reviewed for PR #308 acceptance remediation: Portal validation covers pinned CLI transport, exact apikey/Cookie isolation, lease/cache safety, correlation, safe events, and the exact 144-root inventory.'
related:
  - ../../AGENTS.md
  - ../../.docpact/config.yaml
...
`pnpm check` first requires exact Deno `2.9.5` / bundled TypeScript `6.0.3` plus Node `24.19.0` / pnpm `11.23.0`. It then checks all 144 enabled `supabase/functions/*/index.ts` and `test/*.ts` roots in one shared graph, runs Node contract tests, and executes all 422 Deno behavior tests with only env, read, and loopback-network permissions. Deno is the authoritative compiler; no npm TypeScript package participates.
