...
lastReviewedCommit: 2a02cedd986fe9d6053e59db38b858dac691f6b2
lastReviewedNote: 'Updated for Issue #249 review: hosted Worker proof now has read-only branch/migration/source provenance, the complete service and browser-role matrix, and stricter consumer-zero coverage.'
related:
  - ../../AGENTS.md
  - ../../.docpact/config.yaml
...
| Review-submit numerical gate API, submit job worker, Worker capability repository/API, or submit-review gate assertion | `npm run lint`; `npm run check`; `npm run test:worker-contract:unit`; `npm run test:worker-contract:consumer-zero`; targeted `deno check` on `app_dataset_review_submit_gate`, `app_dataset_review_submit_jobs`, `app_worker_jobs`, `process_dataset_review_submit_jobs`, dataset command files, Worker capability/command files, and compatibility DB-RPC exports; run `test/app_dataset_review_submit_gate_test.ts`, `test/app_dataset_review_submit_jobs_test.ts`, `test/app_worker_jobs_test.ts`, `test/review_submit_job_worker_test.ts`, `test/worker_jobs_test.ts`, `test/app_dataset_submit_review_test.ts`, and `test/dataset_command_rpc_contract_test.ts` | with the matching database-engine worker migration applied to an isolated loopback stack, set `WORKER_CAPABILITY_DB_CONTRACT=1`, `WORKER_CAPABILITY_DB_URL`, and the local Supabase URL/publishable/service keys, then run `npm run test:worker-contract:db`; for the approved isolated hosted Preview, set `WORKER_CAPABILITY_CONTRACT_MODE=hosted-preview`, `WORKER_CAPABILITY_SUPABASE_ACCESS_TOKEN`, and the Preview URL/publishable/service keys, then run `npm run test:worker-contract:hosted`; never target production | The hosted contract does not trust caller-supplied expected provenance. It binds the parent project, branch UUID/name, and Preview ref through the Supabase branch API; proves exact migration head/name and statement receipt through the migration API plus read-only ledger; verifies the migration-generated, postgres-owned residue view and its read-only ACL/definition hash; and binds the matching source branch, merged database-engine PR `#365`, commit `6809528c32bac8163e9a6eec9b985d57370589e1`, and migration-file SHA through GitHub. It then proves all six `api.worker_*_v1` service positives, exact `42501` for anon/foreign/owner/admin across all six RPCs and `api.worker_job_domain_refs`, private-profile denial, authenticated API ownership, cross-user read/cancel denial, and controlled cancellation/user cleanup. The consumer-zero proof covers single-, double-, and backtick-quoted legacy literals and rejects versioned direct calls outside the capability repository. Snapshot lookup must preserve the bounded `created_at DESC, id DESC` candidate scan and Edge-side expiry loop. Do not widen authenticated RPC execution or duplicate worker blocker heuristics in Edge. |
