...
lastReviewedNote: 'Updated for Issue #250: non-direct RPC/schema syntax is default-denied and all allowed syntax categories are exact-counted for the current slice.'
related:
  - ../../AGENTS.md
  - ../../.docpact/config.yaml
...
| PostgREST schema boundary, capability manifest, or public-residue cutover | `npm run audit:schema-boundary:expand`; `deno test --allow-read --config supabase/functions/deno.json test/schema_boundary_capability_test.ts test/schema_boundary_consumer_audit_test.ts`; `npm run check`; targeted tests for every migrated consumer | prove `npm run audit:schema-boundary:contract` exits non-zero with both `database-freeze-pending` and public residue before cutover; with the exact coordinated database-engine #357 v3 freeze in an isolated loopback stack, prove actor/service/anon ACL, ownership, idempotency, CAS, and transaction semantics | API-ready and public-preservation typed maps plus observed literal capability IDs must be bidirectionally consistent with the manifest. An incremental slice may select `api` only for routines proven at the exact bound database merge/migration head; every other capability must remain unqualified/public and emit an explicit contract residue finding. Source-derived `(file, span, relation, operation)` occurrences must be 48/48 with no duplicate, missing, or stale consumer. RPC/schema authorization uses exact-counted syntax tuples for five typed-adapter direct calls, thirteen direct legacy calls, and seven independent `typeof` method guards. Every other non-direct `.rpc`/`.schema` property or element access—including aliases and `bind`/`call`/`apply` chains—fails by default. The thirteen ordinary business `schema` fields must match twelve exact path/symbol/expression/context tuples, and the one non-database computed-call exception is separately exact-counted; missing and duplicate occurrences fail. This is a syntax restriction for the current save-draft slice, not a global consumer-zero claim. Candidate DB metadata is explicitly non-authorizing; do not merge or deploy until the reviewed-frozen v3 digest, exposure signatures, ACL policy bindings, and live contract all match. PR CI checks out and verifies the exact PR head, then runs negative tests, expand audit, expected-red contract assertion, and Deno check. |
