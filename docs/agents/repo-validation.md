...
lastReviewedNote: 'Reviewed for Issue #233: closure artifact proof now covers strict descriptor decoding, XLSX and machine roles, semantic filenames, expiry-capped signing, stable lifecycle errors, and locator-free failures.'
related:
  - ../../AGENTS.md
  - ../../.docpact/config.yaml
...
| Data Product closure commands, certificate-bound result builds, TaskSummaryV2 feed, or closure-artifact signing | `npm run lint`; `npm run check`; `deno test --allow-env --config supabase/functions/deno.json test/data_product_command_test.ts`; targeted `deno check` on `app_data_product_commands`, `_shared/commands/data_product/**`, and `_shared/db_rpc/data_product_commands.ts` | against a local database-engine stack, prove owner/manager and cross-user denial, a v2 build does not double-enqueue its persisted job, feed rows expose no payload/locator/signed URL, and XLSX plus machine manifest/partition signing occurs only for ready unexpired actor-bound descriptors; also prove semantic filenames, the artifact-expiry TTL cap, stable owner-visible `410`, opaque unavailable/unauthorized `404`, and locator-free `502` | Database owns scope normalization, Certificate validity, feed ACL, artifact lifecycle, and artifact authorization. Edge must keep request/descriptor schemas strict, pass the versioned read pages through, and keep bucket/object paths and storage error details inside the service-only signing step. |
