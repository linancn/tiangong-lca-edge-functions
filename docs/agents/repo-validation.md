...
lastReviewedNote: 'Reviewed for Issue #254: adapter proof now requires a distinct scoped fake, identity type rejection, zero root RPC calls, and no default-schema fallback.'
related:
  - ../../AGENTS.md
  - ../../.docpact/config.yaml
...
| LCA solve, queue, result, or scope helpers | `npm run lint`; `npm run check`; targeted `deno check` on changed `lca_*` files and `_shared/lca_*` helpers; run neighboring scope/queue tests. Snapshot capability changes must run `test/lca_snapshot_capability_test.ts`, `test/lca_snapshot_endpoint_handler_test.ts`, `npm run diagnose:lca-snapshot:call-sites`, `test/lca_snapshot_scope_db_test.ts`, and `test/lca_snapshot_build_queue_test.ts`. The handler test invokes the production solve/query/contribution factories and proves exact repository operations, DTO consumption, candidate fallback, and 404/500 behavior. The call-site inventory command only checks six explicit owner files for simple adapter markers; it does not parse TypeScript, infer dynamic consumers, prove consumer-zero, or authorize object retirement. Authorization evidence comes from real handler behavior, Data API role tests, ACL/readback proof, runtime observation, and owner confirmation. When an exact database-engine stack is available, configure the loopback-only `LCA_SNAPSHOT_CONTRACT_*` environment and run `npm run test:lca-snapshot:db`. This checked-in self-enforcing runner executes the DB/Auth contract plus cleanup failure injection, cleans seeded/create rows after revoking the temporary authenticated session and deleting its user, independently reads back rows/users/sessions as zero, and rejects ignored or partially executed tests. The transport matrix must identify the actual Data API roles used and prove all six RPCs reject both anon and a real authenticated access token. Static LCIA contract changes must also run `test/lca_static_cache_bundle_contract_test.ts`, `test/lca_snapshot_scope_test.ts`, and `test/lca_all_unit_solve_queue_test.ts` with `--allow-read` so raw-manifest hash parity and adversarial locator/evidence drift are covered. For worker_jobs cutover changes also run `test/worker_jobs_cutover_test.ts` and `test/worker_jobs_test.ts` | run `scripts/lca_submit_poll_fetch.sh` when the task explicitly touches the submit, poll, or fetch path; otherwise record why that proof is deferred | Snapshot family access is service-role-only through its fixed `api` adapter, with no request-JWT substitution, private access, or public fallback. `worker_jobs` is the default enqueue path; `LCA_WORKER_JOBS_ENABLED=false` must fail closed instead of using legacy queue fallback. Named private scope rejects old combined-scope hashes and v1 database/union LCIA evidence. The static source base URL is worker-trusted configuration, never a client field. Domain rows/cache remain result metadata, not task fact. Missing DB-side truth is validated in `database-engine`, not here. |
