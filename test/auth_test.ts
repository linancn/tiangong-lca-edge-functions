...
Deno.test('Cognito-like bearer tokens are rejected through Supabase verification', async () => {
  ...
  assertEquals(result.isAuthenticated, false);
  assertEquals(result.response?.status, 401);
  assertEquals(await result.response?.text(), 'Invalid JWT issuer');
  assertEquals(getClaimsCalls, 1);
});
...
Deno.test(
  'service auth remains independent and opaque bearers fail Supabase verification',
  async () => {
    await withEnv({ SERVICE_API_KEY: TEST_SERVICE_API_KEY }, async () => {
      ...
      const malformedResult = await module.authenticateRequest(
        new Request('https://example.com', {
          headers: { Authorization: 'Bearer definitely-not-a-broker-or-jwt' },
        }),
        {
          authClient: {
            auth: {
