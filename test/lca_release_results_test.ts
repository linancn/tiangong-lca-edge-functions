...
function repository(label: string, privateFailure = false) {
  return {
    getCurrent: () => Promise.resolve({ ok: true as const, data: { label, status: 'current' } }),
    ...
    createArtifactDownload: (artifactId: string) =>
      Promise.resolve({
        ok: true as const,
        data: { label, artifactId, signedDownloadUrl: 'https://download.example/artifact.zip' },
      }),
  };
}
...
Deno.test('anonymous current release lookup uses the public repository', async () => {
  const handler = createLcaReleaseResultsHandler({ publicRepository: repository('public') });
  const response = await handler(
    new Request('http://localhost/functions/v1/lca_release_results', { method: 'GET' }),
  );
